...
if(isset($_SESSION["timer"]) && $_SESSION["timer"] == 0){
    echo '<script>alert("Time Expired !!"); location.href= "result.php?exam=html"</script>';
}
...
if (isset($_POST["insert"]) && $_POST["insert"] == "yes"){
    ...
    $sql = "UPDATE `html".$_SESSION["lid"]."` SET `answer`='".$ans."',`unvisited` = 1,review ='', `flag` = ".$flag." WHERE `qno`=".$_POST["qno"];
...
if (isset($_POST["review"]) && $_POST["review"] == "yes"){
    $ans = $_POST["ans"];
    $flag = 3;
    $sql = "UPDATE `html".$_SESSION["lid"]."` SET `answer`='".$ans."',`unvisited` = 1, review ='".$ans."' , `flag` = ".$flag." WHERE `qno`=".$_POST["qno"];
    if ($conn->query($sql) === TRUE){
        echo "success";
    }
    exit;
...
<div class="container">
    ...
    <script>
        ...
        function timer(val) {
            if (val < 0){
                alert("Time Up !!");
                location.href = "result.php?exam=html";
            }
            $("#timer").html(val);
            setTimeout(function () {
                $.post('html5.php',{timer:"yes", val: val})
                    .done(function (data) {
                        timer(data);
                    })
            },1000);
        ...
        <div class="col-md-8 col-sm-6 hero-feature">
            <div class="thumbnail">
                <div class="caption">
                    ...
                    $sql = "SELECT * FROM `html` WHERE id =".$qno;
                    $res = $conn->query($sql);
                    $row = $res->fetch_assoc();
                    $sql1 = "SELECT * FROM  `html".$_SESSION["lid"]."` WHERE flag in(2,3) and qno=".$qno;
            ...
            <div class="thumbnail">
                <div class="caption">
                    ...
                    while ($row = $res->fetch_assoc()) {
                        $sql1 = "SELECT * FROM `html".$_SESSION["lid"]."` WHERE qno=".$row["id"];
                        $res1 = $conn->query($sql1);
                        $row1 = $res1->fetch_assoc();
                        switch ($row1["flag"]){
                            case 1:
                        ...
                        ?>
                        <button style="margin-bottom: 2%" class="btn btn-<?=$btn?>" onclick="location.href='html5.php?qno=<?=$row["id"]?>'"><?=$row["id"]?></button>
                        <?php
                    }
                    ?>
                </div>
...
<script>
    ...
    function nextquestion(val) {
        ...
        $.post('html5.php', {ans: ans, qno: val, insert: "yes"})
            .done(function (data) {
                if (data.trim() == "success"){
                    if(parseInt(val)+1 != 11)
                        location.href = "html5.php?qno="+(parseInt(val)+1);
                    else {
                        location.href = "result.php?exam=html";
                    }
                }
    ...
    function review(val) {
        if($("input[name='optradio']:checked").length > 0){
            $.post('html5.php', {ans: $("input[name='optradio']:checked").val(), qno: val, review: "yes"})
                .done(function (data) {
                    if (data.trim() == "success"){
                        if(parseInt(val)+1 != 11)
                            location.href = "html5.php?qno="+(parseInt(val)+1);
